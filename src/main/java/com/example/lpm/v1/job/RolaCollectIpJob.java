...
public class RolaCollectIpJob implements CommandLineRunner {
        ...
        while (true) {
            ...
            try {
                RAtomicLong collectFlag =
                    redissonClient.getAtomicLong(RolaCollectConstant.ROLA_COLLECT_BY_SID_FLAG_KEY);
                ...
                userSb.append(RolaLoadData.CACHE.get(1).getSidUsername()).append("-country-")
                    .append(rolaProxy.getCountry());
                ...
                java.net.Authenticator.setDefault(new java.net.Authenticator() {
                    private final PasswordAuthentication authentication = new PasswordAuthentication(
                        finalUserSb.toString(), RolaLoadData.CACHE.get(1).getProxyPassword().toCharArray());
                ...
                OkHttpClient client = new OkHttpClient.Builder().connectTimeout(3, TimeUnit.SECONDS)
                    .readTimeout(5, TimeUnit.SECONDS).proxy(new Proxy(Proxy.Type.SOCKS,
                        new InetSocketAddress(ipPort.get(0), Integer.parseInt(ipPort.get(1)))))
                    .build();
                ...
                // long count = rolaIpMapper.selectCount(new QueryWrapper<RolaIpDO>().lambda().eq(RolaIpDO::getIp,
                // ip234DTO.getIp()));
                if (bloomFilter.contains(ip234DTO.getIp())) {
                    ...
                    // redisTemplate.opsForList().leftPush(RolaCollectConstant.ROLA_COLLECT_BY_SID_RESULT,
                    // collectResultDTO);

                    RAtomicLong currentRepeatNum =
                        redissonClient.getAtomicLong(RolaCollectConstant.ROLA_COLLECT_BY_SID_DUPLICATE_NUM);
                    ...
                    try {
                        ...
                        RAtomicLong successNum =
                            redissonClient.getAtomicLong(RolaCollectConstant.ROLA_COLLECT_BY_SID_SUCCESS_NUM);
                ...
                log.error(" ROLA_COLLECT_BY_API_QUEUE_KEY interruptedException:{}",
                    ExceptionUtil.stacktraceToString(ie));
    ...
    public void collectByApi() {
        ...
        while (true) {
            ...
            try {
                RAtomicLong collectFlag =
                    redissonClient.getAtomicLong(RolaCollectConstant.ROLA_COLLECT_BY_API_FLAG_KEY);
                if (collectFlag.get() == 11L) {
                    // 11 停止
                    Thread.sleep(3000);
                    continue;
                ...
                Proxy proxy =
                    new Proxy(Proxy.Type.SOCKS, new InetSocketAddress(ipPort.get(0), Integer.parseInt(ipPort.get(1))));
                ...
                if (bloomFilter.contains(luminatiIPDTO.getIp())) {
                    ...
                    // collectResultDTO.setMsg("重复IP");
                    // redisTemplate.opsForList().leftPush(RolaCollectConstant.ROLA_COLLECT_BY_API_RESULT,
                    // collectResultDTO);

                    RAtomicLong currentRepeatNum =
                        redissonClient.getAtomicLong(RolaCollectConstant.ROLA_COLLECT_BY_API_DUPLICATE_NUM);
                    currentRepeatNum.incrementAndGet();
                } else {
                    RolaIpDO rolaIpDO = new RolaIpDO();
                    ...
                    try {
                        ...
                        RAtomicLong successNum =
                            redissonClient.getAtomicLong(RolaCollectConstant.ROLA_COLLECT_BY_API_SUCCESS_NUM);
                        successNum.incrementAndGet();
