...
public class RolaProxyStrategy implements ProxyStrategy {
    ...
    public void changeProxyIp(ChangeIpRequest changeIpRequest) {
        ...
        ProxyPortDO proxyPortDO = proxyPortService.getOne(
            new QueryWrapper<ProxyPortDO>().lambda().eq(ProxyPortDO::getProxyPort, changeIpRequest.getProxyPort()));
        ...
        long count = proxyIpService.count(new QueryWrapper<ProxyIpDO>().lambda()
            ...
            .eq(CharSequenceUtil.isNotBlank(changeIpRequest.getState()), ProxyIpDO::getRegion,
                changeIpRequest.getState())
            .eq(ProxyIpDO::getTypeName, changeIpRequest.getProxyIpType()).ne(ProxyIpDO::getStatus, 0)
            .ne(ProxyIpDO::getIp, proxyPortDO.getIp()));
        if (count > 0) {
            int c = RandomUtil.randomInt(0, (int)count);
            ProxyIpDO proxyIpDO = proxyIpService
                .getOne(new QueryWrapper<ProxyIpDO>().lambda().eq(ProxyIpDO::getCountry, changeIpRequest.getCountry())
                    .eq(CharSequenceUtil.isNotBlank(changeIpRequest.getCity()), ProxyIpDO::getCity,
                        changeIpRequest.getCity())
                    .eq(CharSequenceUtil.isNotBlank(changeIpRequest.getState()), ProxyIpDO::getRegion,
                        changeIpRequest.getState())
                    .eq(ProxyIpDO::getTypeName, changeIpRequest.getProxyIpType()).ne(ProxyIpDO::getStatus, 0)
            ...
            AccountInfoDO accountInfoDO = accountInfoService.getOne(new QueryWrapper<AccountInfoDO>().lambda()
                .eq(AccountInfoDO::getTypeName, changeIpRequest.getProxyIpType()).eq(AccountInfoDO::getStatus, 1));

            String spsResult = ExecuteCommandUtil.executeRolaProxySps(changeIpRequest.getProxyPort(),
                changeIpRequest.getProxyUsername(), changeIpRequest.getProxyPassword(), proxyIpDO.getIp(),
                accountInfoDO.getUsername(), accountInfoDO.getPassword(), accountInfoDO.getServer(),
            ...
            if (ExecuteCommandUtil.proxyState(spsResult)) {
                String localSpsResult = ExecuteCommandUtil.rolaLocalLumtest(changeIpRequest.getProxyPort(),
