...
import com.example.lpm.v1.domain.entity.*;
...
import com.example.lpm.v1.domain.request.*;
...
public class RolaController {
    ...
    public boolean startSocksPort(@RequestBody RolaStartSocksPortRequest startSocksPortRequest) {
        ...
        long count = portWhitelistService.count(new QueryWrapper<PortWhitelistDO>().lambda()
            .eq(PortWhitelistDO::getPort, startSocksPortRequest.getSocksPort()));
        if (count > 0) {
