...
import com.example.lpm.v1.domain.dto.RolaCollectQueueMsgDTO;
...
import com.example.lpm.v1.domain.request.RolaIpActiveRequest;
...
import com.example.lpm.v1.domain.vo.RolaProgressVO;
import com.example.lpm.v1.job.RolaLoadData;
...
public class RolaIpServiceImpl extends ServiceImpl<RolaIpMapper, RolaIpDO> implements RolaIpService {
    ...
    public void collectV2(RolaIpRequest rolaIpRequest) {
        ...
        // 判断 队列数量，>0拒绝任务
        RBlockingQueue<String> queue = redissonClient.getBlockingQueue(RedisKeyConstant.ROLA_COLLECT_IP_QUEUE_KEY_V2);
        if (queue.size() > 0) {
            throw new BizException(ReturnCode.RC999.getCode(), "已有项目在执行，请等待完成后，再次增加收录项目。");
        }
    ...
    public RolaProgressVO collectBySidProgress() {
        ...
        List<Object> rolaCollectResultDTOList =
            redisTemplate.opsForList().range(RolaCollectConstant.ROLA_COLLECT_BY_SID_RESULT, 0, -1);
    ...
    public void collectByApi(RolaCollectRequest rolaCollectRequest) {
        ...
        String url = RolaCollectConstant.GET_IPS_LINK + RolaLoadData.CACHE.get(1).getToken() + "&qty="
            + rolaCollectRequest.getNumber() + "&country=" + rolaCollectRequest.getCountry();
        ...
        OkHttpClient client = new OkHttpClient().newBuilder().addInterceptor(new GzipRequestInterceptor()).build();
    ...
    public RolaProgressVO collectByApiProgress() {
        ...
        List<Object> rolaCollectResultDTOList =
            redisTemplate.opsForList().range(RolaCollectConstant.ROLA_COLLECT_BY_API_RESULT, 0, -1);
    ...
    public PageVO<RolaIpDO> listRolaIpsPage(RolaQuery rolaQuery, PageQuery pageQuery) {
        ...
        List<RolaIpDO> rolaIpDOList =
            rolaIpMapper.selectList(new QueryWrapper<RolaIpDO>().lambda().ne(RolaIpDO::getStatus, 3)
    ...
    public List<RolaIpDO> listByFileFlag(String fileFlag, String fileType) {
        return rolaIpMapper.selectList(new QueryWrapper<RolaIpDO>().lambda().like(RolaIpDO::getFileFlag, fileFlag)
            ...
            .orderByDesc(RolaIpDO::getUpdateTime));
    }
